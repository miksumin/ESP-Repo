...
Единственное ограничение - рестарт прошивки каждые 24 часа. Прошивка собрана для контроллера Wemos D1 mini (NodeMCU). 
Также может быть собрана для других типов контроллеров. Подключение OT адаптера: выход адаптера (входной сигнал контроллера от котла) - пин D2 (GPIO-04), 
...
так и по сети с других устройств, в том числе по протоколам BLE и RF-433 (выносные датчики метеостанций),
...
Стоимость прошивки, включающей стандартный набор функционала для управления котлом по комнатной температуре (WiFi,MQTT,OT,BLE,Thermostat,Config), 2000 руб.
